...
library(conflicted)
library(ggmap)
# library(rmapshaper) this currently fails due to missing v8 library
library(mapview)
library(rgdal)
library(jsonlite)
...
nc_path <- paste0("/scratch/sadamov/COSMO-1/c1_pollen_grid.nc")
```
...
leaflet_map_muni <- leaflet(municipality_prod_geo %>%
  ...
  # ms_simplify()) %>% This requires the rmapshaper package depending on v8
  addTiles(urlTemplate = mymap) %>%
  # Cosmo 1E Domain with extended boundaries
